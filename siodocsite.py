...
pathname = os.environ['HOME'] + '/Documents/siodoc/proc/'
...
pl.subplot(312)
pl.plot(data1,tp,'b-',label='tp')
pl.xticks(visible=False), pl.grid('on')
pl.ylabel('Tp (s)'), pl.legend(loc=2)

pl.subplot(313)
pl.plot(data1,mdir,'b-',label='dp'), pl.xticks(rotation=15)
pl.ylabel('Dp (º)'), pl.legend(loc=2)
pl.axis([data1[0],data1[-1],0,360])
pl.grid('on')
pl.twinx()
pl.plot(data1,wd,'r',label='wd'), 
pl.ylabel('Dir. Vento (º)'), pl.legend(loc=1)
pl.axis([data1[0],data1[-1],0,360])
